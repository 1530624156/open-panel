...
// 判断传进来的数组是否全部为空
function checkAllNullReturn(arr) {
	var null_num = 0;
	for (var i = 0; i < arr.length; i++) {
		if (arr[i] == '' || arr[i] == null) {
			null_num ++;
...
function autoUpdate(idattr,idparam, updatepath) {
...
function loading(status) {
	if (status) {
		var html = '<div class="loading">\n' +
			'        <img src="/static/tool/images/loading.gif" >\n' +
			'    </div>';
		$('body').append(html);
	} else {
		$('div.loading').remove();
...
function getHrefCx(str) {
	...
	str = str.replace(urlReg, function() {
		...
		return '<a url="' + url + '" class="dynacALink">' + arguments[0].replace(/&nbsp;/gi, ' ').trim() + '</a>';
	})
	return str;
}
...
function getJumpWithParams(url, paramsMap) {
	url = url + '?';
	for (var key in paramsMap) {
		url += key + '=' + paramsMap[key] + '&';
	}
...
function elementUiNormalMethod(vue){
	vue['myNotice'] = function (){
		this.$notify({
			title,
			message,
			type,
	...
	vue['myMessage'] = function (type,message,duration){
		if(duration==undefined || duration==null){duration = 1500;}
		this.$message({type,message,duration});
	};

	vue['myAlert'] = function(title,content,yesCb){
	...
	vue['myPrompt'] = function(title,content,value,cb){
	...
	vue['myConfirm'] = function(title,content,yesCb,noCb){
		this.$confirm(content, title, {
			confirmButtonText: '确定',
			cancelButtonText: '取消',
			callback: (e)=>{
				switch(e){
					case 'confirm':yesCb();break;
					case 'cancel':noCb();break;
				}
			}
		});
	};
